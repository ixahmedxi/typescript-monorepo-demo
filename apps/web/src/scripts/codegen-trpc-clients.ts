...
const routers = Object.keys(pkgJson.dependencies).filter((it) =>
  it.startsWith('@org/router'),
)

const TEMPLATE = fs
  .readFileSync(`${__dirname}/codegen-trpc-clients/client.ts`, 'utf-8')
  .toString()
  // remove any '// @ts-expect-error' comments
  .replace(/\/\/ @ts-expect-error.*/g, '')

const TARGET_DIR = `${ROOT_DIR}/apps/web/src/generated-trpc-client`
